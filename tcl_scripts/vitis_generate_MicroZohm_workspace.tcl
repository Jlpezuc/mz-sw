...
## mzcfg is global for now (but can easily be localized being a hash table)
global mzcfg

## System-wide Settings
# Hardware platform: [MicroZohm]
set mzcfg(PLATFORM_NAME) 	MicroZohm

## APU/FreeRTS Settings
# Enable DHCP for automatic assignment of IPv4 address: true, [false]
set mzcfg(ENABLE_DHCP)		false
  ...
  foreach element $tmplist {
    if {[catch {
      if {![string match "*==*" $element] && ! [string match "*NAME*" $element] && ! [string match "" $element]} {
        ...
        puts "Debug: (MZ) $element"
      }
    } result]} {
      puts "Error:(MZ) Script (app_clean) failed failed at $element with: $result."
    }
    incr index
  }
}
...
proc app_build {{name *}} {
  ...
  foreach element $tmplist {
    if {[catch {
      ...
      puts "Error:(MZ) Script (app_build) failed at $element with: $result."
    }
    incr index
  }
}
...
proc vitis_main {} {
  ...
  set PLATFORM_NAME 	$mzcfg(PLATFORM_NAME)
  ...
  puts "Info (MicroZohm): Path to exports to be imported:"
  puts stdout $EXPORT_FOLDER

  ####################################################
  puts "Info (MicroZohm): create Platform Project"
  if {[catch {set XSA_FILES [glob -join -dir ${XSA_FOLDER} *.xsa]} ]} {puts "Error:(MicroZohm) update of ${PLATFORM_NAME} failed: .xsa does not exist in ${XSA_FOLDER}."}
  #
  set XSA_FILE [lindex $XSA_FILES 0]
  puts "WARNING (MicroZohm): Make sure there is only one xsa file in ${XSA_FOLDER} "
  puts "Info (MicroZohm): using {$XSA_FILE}"
  ...
  ####################################################
  puts "Info (MicroZohm): create FreeRTOS_domain"
  #add freertos domain
  domain create -name FreeRTOS_domain -os freertos10_xilinx -proc psu_cortexa53_0

  puts "Info (MicroZohm): change FreeRTOS BSP settings"
  #add liIP lib to BSP
  ...
  platform write
  bsp config dhcp_does_arp_check $mzcfg(ENABLE_DHCP)
  platform write
  bsp config lwip_dhcp $mzcfg(ENABLE_DHCP)
  platform write
  # increase heap size of freertos, to fix javascope glitches
  bsp config total_heap_size 200000000
  platform write

  puts "Info (MicroZohm): regenerate FreeRTOS BSP"
  ...
  #####################################################
  puts "Info (MicroZohm): create Baremetal_domain"
  #create Baremetal domain
  domain create -name Baremetal_domain -os standalone -proc psu_cortexr5_0

  puts "Info (MicroZohm): change Baremetal BSP settings"
  # "Hide" peripherals used by FreeRTOS/Linux from Baremetal(_domain)
  bsp setdriver -ip psu_ethernet_1 -driver generic
  bsp setdriver -ip psu_ethernet_2 -driver generic
  platform write
  ...
  #####################################################
  #puts "Info (MicroZohm): create FSBL_domain"
  ##add FSBL domain
  #domain create -name FSBL_domain -os standalone -proc psu_cortexa53_0
  #
  #puts "Info (MicroZohm): change FSBL BSP settings"
  ...
  #platform write
  #
  #puts "Info (MicroZohm): regenerate FSBL BSP"
  #regenerate board support package
  #bsp regenerate

  #Regenerate platform
  #####################################################
  puts "Info (MicroZohm): generate Platform project"
  platform generate


  #Application Baremetal R5_0
  #####################################################
  puts "Info (MicroZohm): create Baremetal Application"
  # create application
  app create -name Baremetal -template {Empty Application} -platform $PLATFORM_NAME -domain Baremetal_domain

  puts "Info (MicroZohm): import Baremetal Application sources"
  ...
  ####################################################
  puts "Info (MicroZohm): create FreeRTOS Application"
  #create application
  app create -name FreeRTOS -template {Empty Application} -platform $PLATFORM_NAME -domain FreeRTOS_domain

  puts "Info (MicroZohm): import FreeRTOS Application sources"
  #import sources to freertos project

  puts "Info (MicroZohm): Path to FreeRTOS source files:"
  ...
  #####################################################
  #puts "Info (MicroZohm): create FSBL Application"
  ...
  #
  #puts "Info (MicroZohm): import FSBL Application sources"
  ...
  puts "Info (MicroZohm): add standard FSBL.elf"
  ...
  #Clean all
  ####################################################
  puts "Info (MicroZohm): clean platform and all application projects"
  platform clean
  app_clean
  puts "Info (MicroZohm): build platform and all application projects"
  platform generate
  app_build
  ...
  puts "Info (MicroZohm): Path to launches:"
  ...
  puts "========================================"
  puts "Info (MicroZohm): debug files copied"
  puts "Info (MicroZohm): debug configurations are visible after restarting Vitis"
  puts "========================================"
  puts "Info (MicroZohm): generate_MicroZohm_workspace.tcl script finished successfully"
  puts "========================================"
}
