...
uint32_t js_status_BareToRTOS=0;				// Contains (among other things?) the status of the for "MicroZohm LEDs" (cf. ipc_ARM.c):
...
int JavaScope_initalize(DS_Data* data)
{
	...
	// Senales del scope: las fijas del sistema (javascope.h) y las de la aplicacion (globalData.h).
	// Las listas X(...) generan una asignacion js_ch_observable[JSO_<nombre>] = puntero por senal.
#define X(name, ptr) js_ch_observable[JSO_##name] = (ptr);
	MZ_SCOPE_SYSTEM_SIGNALS(X)
#undef X
#define X(name) js_ch_observable[JSO_##name] = &data->sv.name;
	MZ_SCOPE_SIGNALS(X)
#undef X

	// Datos lentos (se envia uno por interrupcion, de forma rotatoria). Solo float.
...
void JavaScope_update(DS_Data* data){
	...
	lifecheck 				= mz_SystemTime_GetInterruptCounter() % 1000;
	ISR_execution_time_us	= mz_SystemTime_GetIsrExectionTimeInUs();
	ISR_period_us			= mz_SystemTime_GetIsrPeriodInUs();
	System_UpTime_seconds   = mz_SystemTime_GetUptimeInSec();
	System_UpTime_ms		= mz_SystemTime_GetUptimeInMs();
