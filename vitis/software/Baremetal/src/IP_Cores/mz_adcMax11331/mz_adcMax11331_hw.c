...
#include "mz_adcMax11331_hwAdresses.h"
#include "../../mz/mz_AXI.h"
...
void mz_adcMax11331_hw_write_spi_cfgr(uint32_t base_address, uint32_t value)
{
	...
	mz_axi_write_uint32(base_address+ADC_MAX11331_CLK_DIVISION,value);
}

void mz_adcMax11331_hw_write_master_channel(uint32_t base_address, uint32_t value)
{
	mz_assert_not_zero(base_address);
	mz_axi_write_uint32(base_address + ADC_MAX11331_ADCSELECTOR_FORCEINIT, value);
}
...
{
	mz_assert_not_zero(base_address);
	return (mz_axi_read_uint32(base_address + ADC_MAX11331_CLK_DIVISION));
}

uint32_t mz_adcMax11331_hw_read_master_channel(uint32_t base_address)
{
	mz_assert_not_zero(base_address);
	return (mz_axi_read_uint32(base_address + ADC_MAX11331_ADCSELECTOR_FORCEINIT));
}
