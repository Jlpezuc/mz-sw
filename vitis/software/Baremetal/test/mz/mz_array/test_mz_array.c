...
void test_mz_array_float_initialization(void)
{
    ...
    mz_array_float_t testarray = {
        .length = MZ_ARRAY_SIZE(data), // ALWAYS use the MZ_ARRAY_SIZE makro in the initialization of the length of the array
        .data = &data[0]}; // Pointer to the first element of the actual data of the array
    TEST_ASSERT_EQUAL_FLOAT_ARRAY(data, testarray.data,MZ_ARRAY_SIZE(data));
}

void test_mz_array_uint32_t_initialization(void)
{
    uint32_t data[5] = {1, 2, 3, 4, 5};
    mz_array_uint32_t testarray = {
        .length = MZ_ARRAY_SIZE(data),
        .data = &data[0]};
    TEST_ASSERT_EQUAL_UINT32_ARRAY(data, testarray.data, testarray.length);
}

void test_mz_array_int32_t_initialization(void)
{
    int32_t data[5] = {-1, 2, -3, 4, -5};
    mz_array_int32_t testarray = {
        .length = MZ_ARRAY_SIZE(data),
        .data = &data[0]};
...
void test_mz_array_int16_t_initialization(void)
{
    int16_t data[5] = {-1, 2, -3, 4, -5};
    mz_array_int16_t testarray = {
        .length = MZ_ARRAY_SIZE(data),
        .data = &data[0]};
    TEST_ASSERT_EQUAL_INT16_ARRAY(data, testarray.data, testarray.length);
}

void test_mz_array_uint16_t_initialization(void)
{
    uint16_t data[5] = {1, 2, 3, 4, 5};
    mz_array_uint16_t testarray = {
        .length = MZ_ARRAY_SIZE(data),
        .data = &data[0]};
