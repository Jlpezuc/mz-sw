...
TEST_FILE("mz_signals_iir_filter.c")

struct mz_IIR_Filter_config config = { 0 };
void setUp(void)
{
...
void test_mz_signals_IIR_Filter_init_assert_sample_Freq(void) {
    config.sample_frequency_Hz = 0.0f;
    TEST_ASSERT_FAIL_ASSERT(mz_signals_IIR_Filter_init(config));
    config.sample_frequency_Hz = -20.0f;
    TEST_ASSERT_FAIL_ASSERT(mz_signals_IIR_Filter_init(config));
}

void test_mz_signals_IIR_Filter_init_assert_cutoff_Freq(void) {
...
void test_mz_signals_IIR_Filter_init_assert_sample_Freq_higher_than_cutoff_Freq(void) {
    ...
    TEST_ASSERT_PASS_ASSERT(mz_signals_IIR_Filter_init(config));
}

void test_mz_signals_IIR_Filter_sample_NULL_pointer(void) {
    float input = 10.0f;
    TEST_ASSERT_FAIL_ASSERT(mz_signals_IIR_Filter_sample(NULL, input));
}

void test_mz_signals_IIR_Filter_reverse_sample_NULL_pointer(void) {
    float input = 10.0f;
    TEST_ASSERT_FAIL_ASSERT(mz_signals_IIR_Filter_reverse_sample(NULL, input));
}

void test_mz_signals_IIR_Filter_lowpass_output(void) {
    ...
    mz_IIR_Filter_t* test_instance = mz_signals_IIR_Filter_init(config);
    for(int i=0;i< (int)(sizeof(input_array) / sizeof(float));i++) {
        filtered_array[i] = mz_signals_IIR_Filter_sample(test_instance, input_array[i]);
        TEST_ASSERT_FLOAT_WITHIN(1e-03f, expected_filtered_array[i], filtered_array[i]);
    }
}

void test_mz_signals_IIR_Filter_highpass_output(void) {
    float input_array[20] = {0.0379f,2.2678f, 0.6551f, -1.3192f, 0.7978f, 
...
void test_mz_signals_IIR_Filter_lowpass_reverse_output(void) {
    ...
    mz_IIR_Filter_t* test_instance = mz_signals_IIR_Filter_init(config);
    for(int i=0;i< (int)(sizeof(input_array) / sizeof(float));i++) {
        filtered_array[i] = mz_signals_IIR_Filter_reverse_sample(test_instance, input_array[i]);
        TEST_ASSERT_FLOAT_WITHIN(1e-03f, expected_filtered_array[i], filtered_array[i]);
    }
}

void test_mz_signals_Filter_1st_highpass_reverse_output(void) {
    float expected_filtered_array[20] = {0.0379f,2.2678f, 0.6551f, -1.3192f, 0.7978f, 
    2.9265f, 1.1755f, -1.0237f, 0.6599f, 2.2902f, -0.0473f, -2.3340f, -0.5630f, 1.3410f,
    -0.8848f, -2.8566f, -1.0677f, 0.9698f, -0.6236f, -2.4335f};
