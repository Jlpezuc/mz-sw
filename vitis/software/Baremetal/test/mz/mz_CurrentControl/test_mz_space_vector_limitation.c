...
void test_mz_CurrentControl_SpaceVector_Limitation_ext_clamping_NULL(void){
    TEST_ASSERT_FAIL_ASSERT(mz_CurrentControl_SpaceVector_Limitation(u_input_Volts,V_dc_volts, max_modulation_index, omega_el_rad_per_sec, i_actual_Ampere, NULL));
}

void test_mz_CurrentControl_SpaceVector_Limitation_V_dc_negative(void){
    V_dc_volts = -5.2f;
    TEST_ASSERT_FAIL_ASSERT(mz_CurrentControl_SpaceVector_Limitation(u_input_Volts,V_dc_volts, max_modulation_index, omega_el_rad_per_sec, i_actual_Ampere, &ext_clamping));
}

void test_mz_CurrentControl_SpaceVector_Limitation_V_dc_zero(void){
    V_dc_volts = 0.0f;
    TEST_ASSERT_FAIL_ASSERT(mz_CurrentControl_SpaceVector_Limitation(u_input_Volts,V_dc_volts, max_modulation_index, omega_el_rad_per_sec, i_actual_Ampere, &ext_clamping));
}

void test_mz_CurrentControl_SpaceVector_Limitation_output(void){
    ...
    for(int i=0;i<11;i++){
        ...
        mz_3ph_dq_t output = mz_CurrentControl_SpaceVector_Limitation(u_input_Volts,V_dc_volts, max_modulation_index, omega_el_rad_per_sec, i_actual_Ampere, &ext_clamping);
		TEST_ASSERT_FLOAT_WITHIN(1e-02, ud_out[i], output.d);
	    TEST_ASSERT_FLOAT_WITHIN(1e-02, uq_out[i], output.q);
    }
}

void test_mz_CurrentControl_SpaceVector_Limitation_output_transition_to_limit(void){
    //Values for comparision from simulation
    V_dc_volts = 12.0f;
...
void test_mz_CurrentControl_SpaceVector_Limitation_output_limited(void){
    //Values for comparision from simulation
    V_dc_volts = 12.0f;
...
void test_mz_CurrentControl_SpaceVector_Limitation_very_low_V_dc_d_axis_prio(void){
...
void test_mz_CurrentControl_SpaceVector_Limitation_very_low_V_dc_q_axis_prio(void){
...
void test_mz_CurrentControl_SpaceVector_Limitation_very_low_V_dc_d_axis_prio_95(void){
...
void test_mz_CurrentControl_SpaceVector_Limitation_very_low_V_dc_q_axis_prio_95(void){
