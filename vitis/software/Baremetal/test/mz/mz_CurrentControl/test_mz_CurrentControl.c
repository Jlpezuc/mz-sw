...
#include "mz_CurrentControl.h"
#include "../mz_piController/mz_piController.h"
#include "../mz_signals/mz_signals.h"
#include "mz_linear_decoupling.h"
#include "../mz_Transformation/mz_Transformation.h"
#include "mz_space_vector_limitation.h"
#include <math.h>

struct mz_CurrentControl_config config = {0};
mz_3ph_dq_t i_actual_Ampere = {0};
mz_3ph_dq_t i_reference_Ampere = {0};
...
void test_mz_CurrentControl_reset_NULL(void){
    TEST_ASSERT_FAIL_ASSERT(mz_CurrentControl_reset(NULL));
}

void test_mz_CurrentControl_set_Kp_id_NULL(void){
    float Kp_id = 10.0f;
    TEST_ASSERT_FAIL_ASSERT(mz_CurrentControl_set_Kp_id(NULL, Kp_id));
}

void test_mz_CurrentControl_set_Kp_id_negative(void){
    float Kp_id = -10.0f;
    mz_CurrentControl_t* instance = mz_CurrentControl_init(config);
    TEST_ASSERT_FAIL_ASSERT(mz_CurrentControl_set_Kp_id(instance, Kp_id));
}

void test_mz_CurrentControl_set_Ki_id_negative(void){
    float Ki_id = -10.0f;
    mz_CurrentControl_t* instance = mz_CurrentControl_init(config);
    TEST_ASSERT_FAIL_ASSERT(mz_CurrentControl_set_Ki_id(instance, Ki_id));
}

void test_mz_CurrentControl_set_Kp_iq_NULL(void){
    float Kp_iq = 10.0f;
    TEST_ASSERT_FAIL_ASSERT(mz_CurrentControl_set_Kp_iq(NULL, Kp_iq));
}

void test_mz_CurrentControl_set_Kp_iq_negative(void){
    float Kp_iq = -10.0f;
    mz_CurrentControl_t* instance = mz_CurrentControl_init(config);
    TEST_ASSERT_FAIL_ASSERT(mz_CurrentControl_set_Kp_iq(instance, Kp_iq));
}

void test_mz_CurrentControl_set_Ki_iq_negative(void){
    float Ki_iq = -10.0f;
    mz_CurrentControl_t* instance = mz_CurrentControl_init(config);
    TEST_ASSERT_FAIL_ASSERT(mz_CurrentControl_set_Ki_iq(instance, Ki_iq));
}

void test_mz_CurrentControl_sample_output(void){
    ...
    for(int i=0;i<11;i++){
        ...
        mz_3ph_dq_t output = mz_CurrentControl_sample(instance, i_reference_Ampere, i_actual_Ampere, V_dc_volts, omega_el_rad_per_sec);
		TEST_ASSERT_FLOAT_WITHIN(1e-02f, ud_out[i], output.d);
	    TEST_ASSERT_FLOAT_WITHIN(1e-02f, uq_out[i], output.q);
    }
}

void test_mz_CurrentControl_sample_abc_output(void) {
    //Values for comparision from simulation
    mz_CurrentControl_t* instance = mz_CurrentControl_init(config);
    float theta_el_rad = (float)M_PI;
    mz_3ph_abc_t output = mz_CurrentControl_sample_abc(instance, i_reference_Ampere, i_actual_Ampere, V_dc_volts, omega_el_rad_per_sec, theta_el_rad);
    ...
    mz_CurrentControl_reset(instance);
    output = mz_CurrentControl_sample_abc(instance, i_reference_Ampere, i_actual_Ampere, V_dc_volts, omega_el_rad_per_sec, theta_el_rad);
...
void test_mz_CurrentControl_sample_output_SVLimitation_active(void){
    V_dc_volts = 10.0f;
    //Values for comparision from simulation
    mz_CurrentControl_t* instance = mz_CurrentControl_init(config);
...
void test_mz_CurrentControl_reset(void){
    //Values for comparision from simulation
    mz_CurrentControl_t* instance = mz_CurrentControl_init(config);
...
void test_mz_CurrentControl_set_Kp_and_Ki_id(void){
    //Tests, if the new values are properly written into the CurrentControl instance
    mz_3ph_dq_t output = {0};
    //Values for comparision from simulation
    mz_CurrentControl_t* instance = mz_CurrentControl_init(config);
    for(int i=0;i<11;i++){
        output = mz_CurrentControl_sample(instance, i_reference_Ampere, i_actual_Ampere, V_dc_volts, omega_el_rad_per_sec);
    }    
    TEST_ASSERT_FLOAT_WITHIN(1e-02f, 6.95f, output.d);
	TEST_ASSERT_FLOAT_WITHIN(1e-02f, 6.95f, output.q);
    mz_CurrentControl_set_Kp_id(instance, 0.0f);
    mz_CurrentControl_set_Ki_id(instance, 0.0f);
    mz_CurrentControl_reset(instance);
    for(int i=0;i<5;i++){
        output = mz_CurrentControl_sample(instance, i_reference_Ampere, i_actual_Ampere, V_dc_volts, omega_el_rad_per_sec);
    }
...
void test_mz_CurrentControl_set_Kp_and_Ki_iq(void){
    //Tests, if the new values are properly written into the CurrentControl instance
    mz_3ph_dq_t output = {0};
    //Values for comparision from simulation
    mz_CurrentControl_t* instance = mz_CurrentControl_init(config);
    ...
    mz_CurrentControl_set_Kp_iq(instance, 0.0f);
    mz_CurrentControl_set_Ki_iq(instance, 0.0f);
    mz_CurrentControl_reset(instance);
    for(int i=0;i<5;i++){
        output = mz_CurrentControl_sample(instance, i_reference_Ampere, i_actual_Ampere, V_dc_volts, omega_el_rad_per_sec);
    }
...
void test_mz_CurrentControl_set_PMSM_parameters(void) {
    mz_3ph_dq_t output = { 0 };
    ...
    mz_CurrentControl_set_PMSM_parameters(instance, config.config_PMSM);
    output = mz_CurrentControl_sample(instance, i_reference_Ampere, i_actual_Ampere, V_dc_volts, omega_el_rad_per_sec);
...
void test_mz_CurrentControl_get_ext_clamping_NULL(void){
    TEST_ASSERT_FAIL_ASSERT(mz_CurrentControl_get_ext_clamping(NULL));
}

void test_mz_CurrentControl_get_ext_clamping_output(void){
    //Values for comparision from simulation
    mz_CurrentControl_t* instance = mz_CurrentControl_init(config);
    i_actual_Ampere.q = 0.0f;
    i_actual_Ampere.d = 0.0f;
    omega_el_rad_per_sec =  0.0f; 
    mz_CurrentControl_sample(instance, i_reference_Ampere, i_actual_Ampere, V_dc_volts, omega_el_rad_per_sec);
    TEST_ASSERT_EQUAL_INT(0,mz_CurrentControl_get_ext_clamping(instance));
    V_dc_volts = 10.0f;
    i_actual_Ampere.q = 0.874f;
    i_actual_Ampere.d = 1.0f;
    omega_el_rad_per_sec =  673.0f; 
    mz_CurrentControl_sample(instance, i_reference_Ampere, i_actual_Ampere, V_dc_volts, omega_el_rad_per_sec);
    TEST_ASSERT_EQUAL_INT(1,mz_CurrentControl_get_ext_clamping(instance));
}

void test_mz_CurrentControl_sample_no_decoupling(void) {
...
void test_mz_CurrentControl_set_decoupling_method(void) {
    config.decoupling_select = no_decoupling;
    config.config_PMSM.Ld_Henry = 0.0f;
    config.config_PMSM.Lq_Henry = 0.0f;
    mz_CurrentControl_t* instance = mz_CurrentControl_init(config);
    TEST_ASSERT_PASS_ASSERT(mz_CurrentControl_sample(instance, i_reference_Ampere, i_actual_Ampere, V_dc_volts, omega_el_rad_per_sec));
    mz_CurrentControl_set_decoupling_method(instance, linear_decoupling);
    //Assertion should trigger now, since for the linear_decoupling Ld/Lq must be greater than 0.0f
    TEST_ASSERT_FAIL_ASSERT(mz_CurrentControl_sample(instance, i_reference_Ampere, i_actual_Ampere, V_dc_volts, omega_el_rad_per_sec));
}

void test_mz_CurrentControl_sample_output_PI_Controller_limit_deactivated(void){
    //Values for comparision from simulation
