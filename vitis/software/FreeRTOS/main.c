...
#include "mz/mz_PLATFORM/mz_platform.h"
#include "mz/mz_PHY_reset/mz_phy_reset.h"
...
int main()
{
	...
	mz_assert( MZ_SUCCESS == mz_platform_init() );
#endif
	...
	mz_printf("APU: Error in scheduler");
...
void network_thread(void *p)
{
    ...
    if ( XST_SUCCESS != mz_platform_macread_primary(mac_ethernet_address) )
        mz_printf("APU: Error fetching MAC address from EEPROM, using default\r\n");
    ...
    mz_printf("\r\n\r\n");
    mz_printf("APU: Network thread started\r\n");
    ...
    if (!xemac_add(netif, &ipaddr, &netmask, &gw, mac_ethernet_address, PLATFORM_EMAC_BASEADDR)) {
		mz_printf("APU: Error adding N/W interface\r\n");
		return;
    }
...
/*	// Enable (currently not required for I²C-based GPOs)
	mz_platform_gposet(I2CLED_FPRING, MZP_GPO_ENABLE2PUSHPULLED);
	mz_platform_gposet(I2CLED_MZD10GREEN, MZP_GPO_ENABLE2PUSHPULLED);
	mz_platform_gposet(I2CLED_MZD11RED, MZP_GPO_ENABLE2PUSHPULLED);
	mz_platform_gposet(I2CLED_MZD12YELLOW, MZP_GPO_ENABLE2PUSHPULLED);
	mz_platform_gposet(I2CLED_MZD13BLUE, MZP_GPO_ENABLE2PUSHPULLED);
*/
	// Set
	mz_platform_gposet(I2CLED_FPRING, MZP_GPO_ASSERT_QUEUED);
	uint8_t mz_lscan = 0;
	mz_platform_gposet(I2CLED_MZD12YELLOW,	MZP_GPO_DEASSERT_QUEUED);
	mz_platform_gposet(I2CLED_MZD13BLUE,	MZP_GPO_DEASSERT_QUEUED);
    ...
    while (1) {
		...
		if( ! hal_can_is_rx_empty() ){
			...
			//	XcpCommand( (uint32_t *) can_frame_rx.data );
				...
				//mz_printf("received a not XCP related CAN frame \n\r");
			}
			//usleep(1000 * 500);
		}else{
			can_send_1();
		...
		mz_platform_gposet(I2CLED_FPRING, MZP_GPO_TOGGLE_QUEUED);

		// Toggle LEDs on MZ sequentially
		switch(mz_lscan)
		{
			default:
				mz_platform_gposet(I2CLED_MZD11RED,		MZP_GPO_DEASSERT_QUEUED);
				mz_platform_gposet(I2CLED_MZD10GREEN,	MZP_GPO_ASSERT_QUEUED);
				mz_lscan = 0;
				break;
			case 1:
				mz_platform_gposet(I2CLED_MZD10GREEN,	MZP_GPO_DEASSERT_QUEUED);
				mz_platform_gposet(I2CLED_MZD11RED,		MZP_GPO_ASSERT_QUEUED);
				break;
			case 2:
				mz_platform_gposet(I2CLED_MZD11RED,		MZP_GPO_DEASSERT_QUEUED);
				mz_platform_gposet(I2CLED_MZD12YELLOW,	MZP_GPO_ASSERT_QUEUED);
				break;
			case 3:
				mz_platform_gposet(I2CLED_MZD12YELLOW,	MZP_GPO_DEASSERT_QUEUED);
				mz_platform_gposet(I2CLED_MZD13BLUE,	MZP_GPO_ASSERT_QUEUED);
				break;
			case 4:
				mz_platform_gposet(I2CLED_MZD13BLUE,	MZP_GPO_DEASSERT_QUEUED);
				mz_platform_gposet(I2CLED_MZD12YELLOW,	MZP_GPO_ASSERT_QUEUED);
...
{
/*	// Enable
	mz_platform_gposet(I2CLED_FP1RDY, MZP_GPO_ENABLE2PUSHPULLED);
	mz_platform_gposet(I2CLED_FP2RUN, MZP_GPO_ENABLE2PUSHPULLED);
	mz_platform_gposet(I2CLED_FP3ERR, MZP_GPO_ENABLE2PUSHPULLED);
	mz_platform_gposet(I2CLED_FP4USR, MZP_GPO_ENABLE2PUSHPULLED);
*/
	while(1) {
		// Mirror "MicroZohm LEDs" (cf. Baremetal/src/sw/javascope.c) to I²C-LEDs
		mz_platform_gposet(I2CLED_FP1RDY, (javascope_data_status & (1<<0)) ? MZP_GPO_ASSERT_QUEUED : MZP_GPO_DEASSERT_QUEUED);
		mz_platform_gposet(I2CLED_FP2RUN, (javascope_data_status & (1<<1)) ? MZP_GPO_ASSERT_QUEUED : MZP_GPO_DEASSERT_QUEUED);
		mz_platform_gposet(I2CLED_FP3ERR, (javascope_data_status & (1<<2)) ? MZP_GPO_ASSERT_QUEUED : MZP_GPO_DEASSERT_QUEUED);
		mz_platform_gposet(I2CLED_FP4USR, (javascope_data_status & (1<<3)) ? MZP_GPO_ASSERT_QUEUED : MZP_GPO_DEASSERT_QUEUED);

		// Push all (I²C-)GPO changes to hardware
		mz_platform_gpoupdate();
...
int main_thread()
{
	...
	mz_printf("APU Build Date: %s at %s,\r\n",__DATE__, __TIME__);
    ...
    while (1) {
		...
		if (server_netif.ip_addr.addr) {
			mz_printf("APU: DHCP request success\r\n");
		...
		if (mscnt >=7500) { // define timeout time here
			mz_printf("APU: DHCP request timed out\r\n");
			mz_printf("APU: Configuring default IP of 192.168.1.233\r\n");
...
{
	mz_printf("std_id: 0x%03X, dlc: %d, data[0]: 0x%02X \n\r",
