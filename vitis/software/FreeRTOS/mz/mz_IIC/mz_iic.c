...
#define MZ_IIC_MAX_BUSINSTANCES	(1U)

// Bus instances (NOT device instances)
static XIicPs instances[MZ_IIC_MAX_BUSINSTANCES];
...
void mz_iic_initbus(uint8_t businst, uintptr_t xlnx_baseaddress, uint16_t xlnx_deviceid, uint32_t xlnx_inputclock_hz, int busfreq_khz) {
	mz_assert(businst < MZ_IIC_MAX_BUSINSTANCES);
...
void mz_iic_initdev(mz_iic *self, uint8_t businst, uint8_t devaddr) {
	...
	mz_assert(businst < MZ_IIC_MAX_BUSINSTANCES);
	XIicPs *iicbus_p = &instances[businst];
	mz_assert(iicbus_p->IsReady);

	mz_assert(devaddr < 128);
...
uint32_t mz_iic_write_reg16(struct mz_iic_ *self, uint8_t regaddr, uint16_t data) {
	...
	return( mz_iic_write_raw(self, txbuf, sizeof(txbuf)/sizeof(txbuf[0])) );
}
...
uint32_t mz_iic_a8read_data(struct mz_iic_ *self, uint8_t regaddr, uint8_t* data, int32_t cnt) {
	mz_assert_not_NULL(self);
	mz_assert(self->is_ready);
	XIicPs *iicbus_p = &instances[self->businst];
...
uint32_t mz_iic_a16read_data(struct mz_iic_ *self, uint16_t regaddr, uint8_t* data, int32_t cnt) {
	mz_assert_not_NULL(self);
	mz_assert(self->is_ready);
	XIicPs *iicbus_p = &instances[self->businst];
